...
public:
										...
										TString DrawOp1 = "EP", TString DrawOp2 = "EPSAME")
	{
		...
		// -- flat line = 1.00 -- //
		TF1 *_f__line = new TF1("_f__line", "1", -10000, 10000);
		_f__line->SetLineColor(kRed);
		_f__line->SetLineWidth(1);
		_f__line->Draw("SAME");
	...
	void CanvasWithTHStackRatioPlot_with_unc(TH1D *h1, THStack *h_stack, vector< TH1D* > histos, TLegend* legend, TString Name_Ratio, Int_t isGrid = 0,
									Int_t color1 = kBlack, Bool_t isFill1 = kFALSE,
									TString DrawOp1 = "EP", TString DrawOp2 = "HIST SAME")
	{
		c->cd();
		...
		// -- for unc. -- //
		TGraphAsymmErrors *gr = new TGraphAsymmErrors(h_ratio);
		Int_t nBin = h_ratio->GetNbinsX();

		for(int i=0;i<nBin;i++)
		{
			TH1D *h_up = (TH1D*)h1->Clone();
			h_up->Divide(h_up, histos[1]);

			TH1D *h_down = (TH1D*)h1->Clone();
			h_down->Divide(h_down, histos[2]);

			Double_t err_up = h_up->GetBinContent(i+1) - h_ratio->GetBinContent(i+1);
			Double_t err_down = -(h_down->GetBinContent(i+1) - h_ratio->GetBinContent(i+1));

			gr->SetPointEYhigh(i,err_up);
			gr->SetPointEYlow(i,err_down);
		}
		...
		// -- Add unc. legend -- //
		TLegend *ll = new TLegend(Legend_x1, Legend_y1, Legend_x2, Legend_y2);
		ll->SetFillStyle(0);
		ll->SetBorderSize(0);
		ll->AddEntry(gr, "prefiring unc.");
		ll->Draw("same");
	...
	void CanvasWithThreeHistogramsRatioPlot(TH1D *h1, TH1D *h2, TH1D* h_ref, 
											...
											Int_t color1 = kRed, Int_t color2 = kBlue, Int_t color_ref = kBlack,
	...
	void CanvasWithThreeHistogramsRatioPlot_rev(TH1D *h1, TH1D *h2, TH1D* h_ref, 
											TString Name1, TString Name2, TString Name_ref, TString Name_Ratio,
											//Int_t color1 = kBlue, Int_t color2 = kGreen+1, Int_t color_ref = kRed,
											//Int_t color1 = kBlue, Int_t color2 = kGreen+1, Int_t color_ref = kBlack,
											Int_t color1 = kRed, Int_t color2 = kBlue, Int_t color_ref = kBlack,
											Bool_t isFill1 = kFALSE, Bool_t isFill2 = kFALSE, Bool_t isFill_ref = kFALSE,
											TString DrawOp1 = "EP", TString DrawOp2 = "EPSAME", TString DrawOp_ref = "EPSAME")
	{
		...
		h_ratio1->SetName("h_ratio1");
		//h_ratio1->Divide(h1, h_ref);
		h_ratio1->Divide(h_ref, h1);

		h_ratio2 = (TH1D*)h1->Clone();
		h_ratio2->SetName("h_ratio2");
		//h_ratio2->Divide(h2, h_ref);
		h_ratio2->Divide(h_ref, h2);
	...
	void CanvasWithFourHistogramsRatioPlot_rev(TH1D *h1, TH1D *h2, TH1D *h3, TH1D* h_ref, 
											TString Name1, TString Name2, TString Name3, TString Name_ref, TString Name_Ratio,
											Int_t color1 = kRed, Int_t color2 = kBlue, Int_t color3 = kGreen, Int_t color_ref = kBlack,
											Bool_t isFill1 = kFALSE, Bool_t isFill2 = kFALSE, Bool_t isFill3 = kFALSE, Bool_t isFill_ref = kFALSE,
											TString DrawOp1 = "EP", TString DrawOp2 = "EPSAME", TString DrawOp3 = "EPSAME", TString DrawOp_ref = "EPSAME")
	{
		c->cd();
		// -- Top Pad -- //
		TopPad = new TPad("TopPad", "TopPad", 0.01, 0.01, 0.99, 0.99);
		...
		if( isFill3 == kTRUE ) h3->SetFillColor(color3);
		else h3->SetFillColorAlpha(kWhite, 0);
		...
		h_ratio3 = (TH1D*)h1->Clone();
		h_ratio3->SetName("h_ratio3");
		//h_ratio3->Divide(h3, h_ref);
		h_ratio3->Divide(h_ref, h3);
		...
		h_ratio3->SetLineColor(color3);
		h_ratio3->SetMarkerStyle(20);
		h_ratio3->SetMarkerSize(1);
		h_ratio3->SetMarkerColor(color3);
		h_ratio3->SetStats(kFALSE);
	...
	void CanvasWithMultipleHistograms(vector< TH1D* > Histos, vector< TString > Names, TString DrawOp = "EP")
	{
		...
		// -- flat line = 0.00 -- //
		TF1 *f_line = new TF1("f_line", "0", -10000, 10000);
		//f_line->SetLineColor(kRed);
		f_line->SetLineColor(kBlack);
		f_line->SetLineWidth(1);
		f_line->Draw("SAME");
